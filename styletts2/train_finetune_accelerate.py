...
def main(config_path):
	...
	# print(log_dir)
	# voice = os.path.split(os.path.split(log_dir)[0])[1]
	...
	rolling_model_retention_count = config.get('rolling_model_retention_count', 10000) # arbitrarily high number
	...
	for epoch in range(start_epoch, epochs):
		...
		with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch+1}/{epochs}") as pbar:
			...
			if (epoch + 1) % save_freq == 0 :
				...
				def sort_pth_key(filename):
					match = re.search(r'epoch_2nd_(\d+)\.pth', filename)
					return int(match.group(1)) if match else 0

				path_files = sorted([os.path.join(log_dir, f) for f in os.listdir(log_dir) if f.endswith('.pth')], key=sort_pth_key)

				if len(path_files) > rolling_model_retention_count:
					while len(path_files) > rolling_model_retention_count:
						deleteFile = path_files.pop(0)

						print(f"Number of files is greater than {rolling_model_retention_count}, removing {deleteFile}.")

						if os.path.exists(deleteFile):
							os.remove(deleteFile)
							print(f"File {deleteFile} has been removed.")
